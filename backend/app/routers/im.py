...
def _get_user_by_im_id(db: Session, im_user_id: str, platform: str = "dingtalk") -> User | None:
    ...
    仅查询独立的 IM 用户映射表，不回退到用户名匹配，防止越权。
    """
    if not im_user_id:
        return None
...
async def wecom_webhook(
    ...
    if request.method == "GET":
        ...
        # 对 echostr 做 XML 转义，防止注入
        echostr_raw = query_params.get("echostr", "")
        echostr_escaped = echostr_raw.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("]]>", "]]&gt;")
        raw_body = f"<xml><Encrypt><![CDATA[{echostr_escaped}]]></Encrypt></xml>".encode()
