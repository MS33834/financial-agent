...
def test_local_upload_and_download_bytes(tmp_path):
    """正常上传/下载应能完整往返."""
...
def test_local_upload_strips_leading_slash(tmp_path):
    """key 以 / 开头应被去除."""
    with patch("app.storage.get_settings") as mock_settings:
        mock_settings.return_value.storage_local_root = str(tmp_path / "store")
        client = LocalStorageClient()
...
def test_local_upload_rejects_dotdot_in_key(tmp_path):
...
def test_local_upload_rejects_path_escape_via_resolve(tmp_path):
    ...
    with patch.object(type(file_path), "resolve", autospec=True) as mock_resolve:
        ...
        def _side_effect(instance, *args, **kwargs):
...
def test_local_upload_wraps_oserror(tmp_path):
...
def test_local_download_rejects_dotdot_in_key(tmp_path):
...
def test_local_download_rejects_path_escape_via_resolve(tmp_path):
...
def test_local_download_nonexistent_raises(tmp_path):
...
def test_local_download_wraps_oserror(tmp_path):
...
def test_minio_init_without_endpoint_raises():
...
def test_minio_ensure_bucket_creates_when_missing():
    """bucket 不存在时应创建."""
...
def test_minio_ensure_bucket_already_exists():
    """bucket 已存在时不应重复创建."""
    fake_minio = MagicMock()
...
def test_minio_ensure_bucket_s3_error_raises():
    """S3Error 应被包装为 StorageClientError."""
    from minio.error import S3Error

    fake_minio = MagicMock()
    fake_minio.bucket_exists.side_effect = S3Error("x", "x", "x", "x", "x", "x")
    with _patch_minio(fake_minio), patch("app.storage.get_settings") as mock_settings:
        mock_settings.return_value.minio_endpoint = "minio.local:9000"
        mock_settings.return_value.minio_access_key = "ak"
        mock_settings.return_value.minio_secret_key = "sk"
...
def test_minio_upload_bytes_success():
...
def test_minio_upload_bytes_uses_endpoint_as_fallback():
...
def test_minio_upload_s3_error_raises():
    """S3Error 应被包装为 StorageClientError."""
    from minio.error import S3Error

    fake_minio = MagicMock()
    fake_minio.bucket_exists.return_value = True
    fake_minio.put_object.side_effect = S3Error("x", "x", "x", "x", "x", "x")
    with _patch_minio(fake_minio), patch("app.storage.get_settings") as mock_settings:
        mock_settings.return_value.minio_endpoint = "minio.local:9000"
        mock_settings.return_value.minio_access_key = "ak"
        mock_settings.return_value.minio_secret_key = "sk"
...
def test_minio_download_bytes_success():
    """成功下载应返回字节内容并释放连接."""
...
def test_minio_download_s3_error_raises():
    """下载 S3Error 应被包装."""
    from minio.error import S3Error

    fake_minio = MagicMock()
    fake_minio.bucket_exists.return_value = True
    fake_minio.get_object.side_effect = S3Error("x", "x", "x", "x", "x", "x")
    with _patch_minio(fake_minio), patch("app.storage.get_settings") as mock_settings:
        mock_settings.return_value.minio_endpoint = "minio.local:9000"
        mock_settings.return_value.minio_access_key = "ak"
        mock_settings.return_value.minio_secret_key = "sk"
...
def test_get_storage_client_local_default():
...
def test_get_storage_client_minio_backend():
...
def test_base_storage_client_abstract_methods_raise():
