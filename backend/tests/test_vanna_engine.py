...
def _patched_engine(monkeypatch, client: MagicMock | None = None) -> VannaEngine:
...
def test_is_available_false_when_vanna_missing(monkeypatch) -> None:
    monkeypatch.setattr(VannaEngine, "_check_installed", staticmethod(lambda: False))
    engine = VannaEngine()
    assert engine.is_available is False


def test_generate_sql_raises_when_unavailable(monkeypatch) -> None:
    monkeypatch.setattr(VannaEngine, "_check_installed", staticmethod(lambda: False))
    engine = VannaEngine()
...
def test_train_ddl_delegates_to_client(monkeypatch) -> None:
    client = MagicMock()
...
def test_train_sql_with_question(monkeypatch) -> None:
    client = MagicMock()
    engine = _patched_engine(monkeypatch, client)
...
def test_train_sql_without_question(monkeypatch) -> None:
    client = MagicMock()
    engine = _patched_engine(monkeypatch, client)
...
def test_train_documentation_delegates(monkeypatch) -> None:
    client = MagicMock()
    engine = _patched_engine(monkeypatch, client)
...
def test_generate_sql_success(monkeypatch) -> None:
    client = MagicMock()
...
def test_generate_sql_returns_none_on_failure(monkeypatch) -> None:
    client = MagicMock()
...
def test_generate_sql_returns_none_on_empty(monkeypatch) -> None:
    client = MagicMock()
...
def test_generate_sql_returns_none_on_non_string(monkeypatch) -> None:
    client = MagicMock()
    client.generate_sql.return_value = 123  # type: ignore[assignment]
    engine = _patched_engine(monkeypatch, client)
    assert engine.generate_sql("营收") is None


def test_get_training_data_returns_records(monkeypatch) -> None:
    client = MagicMock()
    df = MagicMock()
...
def test_get_training_data_empty_on_error(monkeypatch) -> None:
    client = MagicMock()
    df = MagicMock()
...
def test_remove_training_data_success(monkeypatch) -> None:
    client = MagicMock()
    engine = _patched_engine(monkeypatch, client)
...
def test_remove_training_data_false_on_failure(monkeypatch) -> None:
    client = MagicMock()
...
def test_reset_clears_client(monkeypatch) -> None:
...
def test_is_available_module_function(monkeypatch) -> None:
    monkeypatch.setattr(VannaEngine, "_check_installed", staticmethod(lambda: True))
